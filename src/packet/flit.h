...
inline bool is_up_port(uint8_t p)   { return p >= NUM_DN_PORTS; }
inline bool is_down_port(uint8_t p) { return p <  NUM_DN_PORTS; }
...
struct Flit {
    FlitType type    = HEADER;
    uint32_t data    = 0;   // 32 bits de dado/protocolo
    // dst_id extraído do header: data[9:0]
    // src_id mantido para facilitar debug/testbench
    uint8_t  src_id  = 0;
    uint64_t timestamp = 0; // ciclo de injeção (para medir latência)
    ...
    std::string to_string() const {
        ...
        char buf[80];
        if (type == HEADER)
            snprintf(buf, sizeof(buf), "[HEADER src=%d dst=%d data=0x%08X]",
                     src_id, dst_id(), data);
        else
            snprintf(buf, sizeof(buf), "[%s src=%d data=0x%08X]",
                     tn[type], src_id, data);
        return buf;
    }
};
